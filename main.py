...
option_ugb = "density" #density, data_urba, predict_urba. Résultats actuels avec predict_urba
...
for city in np.delete(np.unique(list_city.City), 153): #[108:152]
#for city in ["Athens", "Brisbane", "Los_Angeles"]:
    ...
    #warsaw, yekaterinburg, medellin, mar del plata, helsinki, bucharest, bergen, barcelona
    df = pd.DataFrame(data = np.array([grille.XCOORD, grille.YCOORD, density, simul_density]).transpose())
    df.columns = ["X", "Y", "density", "simul_density"]
    df.to_csv('C:/Users/charl/OneDrive/Bureau/bergen.csv')
    max_d = np.fmax(max(simul_density), max(density))
    
    plt.figure()
    plt.scatter(grille.XCOORD, grille.YCOORD, c = density, cmap='Reds')
    plt.colorbar()
    plt.title(city)
    plt.clim(0, max_d) 
    plt.axis('off')
    ...
    plt.figure()
    plt.scatter(grille.XCOORD, grille.YCOORD, c = simul_density, cmap='Reds')
    plt.colorbar()
    plt.title(city) 
    plt.clim(0, max_d) 
    plt.axis('off')
